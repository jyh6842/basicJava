...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 * - 두개 이상의 클래스를 만드는데 공통된 멤버가 잇는 경우 부모클래스로 만든다.
		 * 
		 * << super >>
		 * - 부모 클래스의 인스턴스 주소가 저장된 변수이다.
		 * - 부모 클래스의 멤버와 지식 클래스의 멤버의 이름이 중복될때 둘을 구분하기 위해 사용한다.
		 * 
		 * << super() >>
		 ...
		 * - 상속받은 메소드와 이런타입, 메소드명, 파라미터가 같아야 한다.
		 * - 상속받은 메소드보다 좋은 범위의 접근제어자로 변경할수 없다.
		 ...
		 * - 부모와 자식에 동인한 멤버변수가 잇는 경우 부모 타입에서는 부모의 변수를 자식타입에서는 자식의 변수를 사용한다.
		 * 
		 * << 접근제어자 >>
...
//		time.setMinute(47);
		...
		System.out.println(Singleton.getInstance()); // 객체 하나로 계속 사용하고 있다.
