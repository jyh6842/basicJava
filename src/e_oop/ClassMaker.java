package e_oop;

public class ClassMaker {
	
	//전역변수 하나를 선언 및 초기화 해주세요.
	int a = 50;
	
	// 리턴타입과 파라미터가 없는 메소드 하나를 만들어주세요.
	// 메소드 안에서 전역변수를 출력해주세요
	void method1(){
		System.out.println("method1 : "+a);
	}//method1()
	
	
	// 전역변수와 동일한 타입의 리턴차입이 있고 파라미터는 없는 메소드 하나를 만들어주세요.
	// 메소드 안에서 전역변수를 리턴해주세요
	int method2(){
		return a;
	}//method2()
	
	// 리턴타입은 업고 파라미터가 있는 메소드 하나를 만들어주세요
	// 메소드 안에서 파라미터를 출력해주세요.
	void method3(int parameter){
		System.out.println("파라미터 : "+parameter);
	}
	
	//int타입의 리턴타입과 int 타입의 파리미터 두개가 있는 메소드 하나를 만들어주세요.
	//메소드 안에서 두 파라미터를 곱합 결과를 리턴해주세요
	int method4(int a, int b){
		return a*b;
	}
	
}// ClassMaker
